...
<body class="">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="card">
                ...
                <div class="card-header card-header-primary" onclick="prjinfo(${prj.pjno});">
                  <input type="hidden" id="${prj.tno}" />
                  <h4 class="card-title ">${prj.tno}팀 상세정보</h4>
                ...
                <div class="card-body">
                  <div class="table-responsive">
                    <table class="table table-hover">
                      ...
                      <thead class="text-primary">
                      	<tr>
                      	  <th>사원번호</th><th>이름</th><th>권한</th><th>개인진행단계</th>
                      	</tr>
                      </thead>
                      <tbody>
                      	<c:forEach var="team" items="${teamDt}">
                        <tr onclick="tmDetail(${team.empno});">
                          <td>${team.empno}</td>
                          <td>${team.ename}</td>
                          <td>${team.position}</td>
                          <td>${team.indistage == '' ? '-' : team.indistage}</td>
                        </tr>
                        </c:forEach>
                      </tbody>
                    </table>
                  ...
                  <div align="right">
                  	<input type="button" class="btn btn-primary" value="팀원추가" id="addMemBtn" />
                  </div>
                </div>
              </div>
            </div>
  ...
  <script>
 	// 팀원 진행상황
 	function tmDetail(empno){
		var tno = $("input[type=hidden]").attr("id");
	 	//$(location).attr("href", "${path}/team.do?method=showWork&empno="+empno+"&tno="+tno);
	 	$(location).attr("href", "${path}/main.do?method=showWork&empno="+empno+"&tno="+tno);
  	}
 	// 프로젝트 조회
	function prjinfo(pjno){
  ...
  $(document).ready(function() {
	  // 팀원 추가
  	  $("#addMemBtn").click(function(){
		  var tno = $("input[type=hidden]").attr("id");
		  $(location).attr("href", "${path}/main.do?method=memList&tno="+tno);
	  });
    $().ready(function() {
        $sidebar = $('.sidebar');
