      ...
      <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
        <div class="container-fluid">
          <div class="navbar-wrapper">
            <a class="navbar-brand" href="#pablo">메세지 작성</a>
          </div>
                      ...
                      <thead class=" text-primary">
                      	<tr>
                      	  ...
                      	  <c:forEach var="emp" items="${elist }">
                      	  	<option value="${emp.empno }">${emp.empno }번 ${emp.ename }(${emp.posi })</option>
                      	  </c:forEach>
                      	  </select>
                      	  </td>
                      	</tr>
                      	...
                      	<tr>
                      	  <th>내용</th><td><textarea rows="10" name="content" class="form-control"></textarea></td>
                      	</tr>
                      	<tr><td colspan="2"><input type="button" id="backBtn" class="btn btn-primary" value="초기"/><input type="button" id="wrtBtn" class="btn btn-warning" value="보내기"/></td></tr>
                      </thead>
                    </table>
                  </form>
                  </div>
  ...
  <script>
    $(document).ready(function() {
    	$("#backBtn").click(function(){
			$(location).attr("href","${path}/main.do?method=messageList");
    	});
    	$("#wrtBtn").click(function(){
    		if(confirm("메세지를 보내겠습니까?")){
    			if($("[name=gno]").val()=='' || $("[name=title]").val()==''|| $("[name=content]").html=='' ){
    				alert("항목을 작성해 주세요");
    			}else{
		    		alert("메세지를 보냈습니다");
		    		$("form").attr("action","${path}/main.do?method=messageSend");
		    		$("form").submit();
    			}
    		}
    	});
