...
<body class="">
      ...
      <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
        <div class="container-fluid">
          <div class="navbar-wrapper">
            <a class="navbar-brand" href="#pablo">업무 리스트</a>
          </div>
            ...
            <ul class="navbar-nav">
              ...
              <li class="nav-item dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdownMenuLink">
                  <a class="dropdown-item" href="${path }/main.do?method=messageList">메세지 함</a>
                </div>
              </li>
              <li class="nav-item dropdown">
            ...
            <div class="col-lg-12 col-md-12">
              <div class="card">
                <div class="card-header card-header-warning">
                  ...
                  <p class="card-category"> 진행중인 프로젝트 번호 : ${work.pjno }</p>
                  <p class="card-category"> 진행중인 프로젝트 명 : ${work.pjtitle }</p>
                  <c:choose>
                  	<c:when test="${work.indistage==0}">
                  		<p class="card-category"> 진행중인 업무가 없습니다</p>
					</c:when>
					<c:otherwise>
                  		<p class="card-category"> 전체 프로젝트 단계 : ${work.allstage }단계</p>
                  		<p class="card-category"> 현재 업무 단계 : ${work.indistage }단계</p>
                  		<p class="card-category"> 파견회사 : ${work.comname }</p>
                  		<p class="card-category"> 파견지역 : ${work.area }</p>
					</c:otherwise>
                  </c:choose>
                </div>
                <div class="card-body">
                  <div class="table-responsive form-group">
                    <table class="table">
                      <col width="9%"><col width="9%"><col width="9%"><col width="9%"><col width="9%"><col width="9%"><col width="9%"><col width="9%"><col width="9%"><col width="9%">
                      <thead class=" text-warning">
                      	<tr>
                      	  <th>프로젝트 번호</th><th>업무 번호</th><th>업무 단계</th><th>업무 정보</th><th>시작일</th><th>종료일</th><th>제출 유무</th><th>완료 유무</th><th>파일 관리</th><th>수정</th><th>삭제</th>
                      	</tr>
                      </thead>
                      <c:forEach var="wlist" items="${wlist }">
                  		<form method="post" id="${wlist.wno }">
                          <input type="hidden" name="empno" value="<%=empno%>"/>
                          <input type="hidden" name="wno" value="${wlist.wno }"/>
                        <tr>
                          <td>${wlist.pjno }</td>
                          <td>${wlist.wno }</td>
                          <td>${wlist.stage }</td>
                          <td><input type="text" class="form-control" name="winfo" value="${wlist.winfo }"/></td>
                          <td><input type="date" class="form-control" name="stdte" value="${wlist.stdte}"/></td>
                          <td><input type="date" class="form-control" name="eddte" value="${wlist.eddte}"/></td>
                          <td><input type="text" class="form-control" name="submi" value="${wlist.submi }"/></td>
                          <td><input type="text" class="form-control" name="comple" value="${wlist.comple}"/></td>
                          <td class="td-actions">
                          	<button type="button" class="btn btn-primary btn-link btn-sm" onclick="file_manage(${wlist.wno });">
                              <i class="material-icons">help_outline</i>
                          	</button>
                          </td>
                          <td><input type="button" class="btn btn-warning" value="수정" onclick="change(${wlist.wno});"/></td>
                          <td><input type="button" class="btn btn-danger" value="삭제" onclick="del(${wlist.wno});"/></td>
                        </tr>
						</form>
  ...
  function change(wno){
	  if(confirm("수정하시겠습니까?")){
		$("#"+wno).attr("action","${path}/team.do?method=workUpt");
		$("#"+wno).submit();
		alert("수정 되었습니다");
	  }
	};
  function del(wno){
	  if(confirm("삭제하시겠습니까?")){
		$(location).attr("href","${path}/team.do?method=workDel&wno="+wno+"&empno="+<%=empno%>);
		alert("삭제 되었습니다");
	  }
  }
  function file_manage(wno){
	  $(location).attr("href","${path}/team.do?method=fileManage&wno="+wno);
  }
